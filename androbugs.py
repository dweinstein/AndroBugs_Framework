...
class Writer :
        ...
        if (self.__cache_output_detail_stream) and (self.__output_current_tag != "") :   
            ...
            if current_tag in self.__output_dict_vector_result_information :
                ...
                try :
                    ...
                    if DEBUG:
                        eprint("[KeyError on \"self.__output_dict_vector_result_information\"]")
                    pass
    ...
    def save_result_to_file(self, output_file_path, args) :
        ...
        try :
            ...
            eprint("<<< Analysis report is generated: " + os.path.abspath(output_file_path) + " >>>")
            eprint("")

            return True
        except IOError as err:
            if DEBUG :
                eprint("[Error on writing output file to disk]")
            return False
...
def __persist_db(writer, args) :
    ...
    if not os.path.isfile(db_config_file) :
        eprint("[ERROR] AndroBugs Framework DB config file not found: " + db_config_file)
        traceback.print_exc()
    ...
    # pymongo.errors.BulkWriteError, pymongo.errors.CollectionInvalid, pymongo.errors.CursorNotFound, pymongo.errors.DocumentTooLarge, pymongo.errors.DuplicateKeyError, pymongo.errors.InvalidOperation
        ...
        try :
            ...
            if DEBUG :
                eprint("[Error on writing Exception to MongoDB]")
                traceback.print_exc()
...
def __persist_file(writer, args) :
    ...
    if package_name and signature_unique_analyze :
        ...
        eprint("\"package_name\" or \"signature_unique_analyze\" not exist.")
        return False
